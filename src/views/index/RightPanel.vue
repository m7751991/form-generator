    ...
    <div class="field-box">
      ...
      <el-scrollbar class="right-scrollbar">
        ...
        >
          ...
          <el-form-item v-if="activeData.text !== undefined" label="文本">
            <el-input
              :value="setDefaultValue(activeData.text)"
              placeholder="请输入默认值"
          ...
          <template
            v-if="
              activeData.__config__.componentName === 'headerComponents' &&
              activeData.trendsProps.length > 0
            "
          >
            <el-button @click="addTrendsProps"> 新增属性 </el-button>
            <el-form-item
              v-for="(item, key) of activeData.trendsProps"
              :key="key"
              :label="item.key"
            >
              <el-input v-model="item.key" placeholder="请输入字段名(v-bind)" />
              <el-input
                v-model="item.value"
                placeholder="请输入字段表达式（v-bind）"
              />
            </el-form-item>
          </template>
...
export default {
  ...
  methods: {
    addTrendsProps() {
      this.activeData.trendsProps.push({
        key: "text",
        value: "{{item.name}}",
      });
    },
    ...
    onTextValueInput(value) {
      this.$set(this.activeData, "text", value);
    },
    setDefaultValue(val) {
