...
function formatProps(scheme){
  let result = []
  let props =scheme.props
  let trendsProps = scheme.trendsProps
  for (const k in props) {
    const v = props[k]
    result.push(`${k} = ${v}`) 
  }
  for (const prop of trendsProps) {
    let  str = `:${prop.key} = '${prop.value}'`
    result.push(str)
  }  
  return result.join(' ')
}
const layouts = {
  renderComponents(scheme){
    const config = scheme.__config__;
    const props = formatProps(scheme)
    return`<${config.tag} class=${scheme.style.className} ${props}></${config.tag}>`;
  },
  textItem(scheme) {
    const config = scheme.__config__;
    return`<div class=${scheme.style.className}>${scheme.text}</div>`;
  },
  colFormItem(scheme) {
    const config = scheme.__config__
    let labelWidth = ''
